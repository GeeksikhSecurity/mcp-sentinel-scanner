...
class UnifiedScanner:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # Initialize tools
        self.mcp_scanner = MCPSentinelScanner(config, parallel_workers=self.config.get("parallel_workers", 4))
    ...
    def _reduce_false_positives(self, findings: List[VulnerabilityFinding]) -> List[VulnerabilityFinding]:
        """Apply false positive reduction."""
    ...
    def _create_summary(self, target: Path, findings: List[VulnerabilityFinding], scan_time: float) -> ScanSummary:
        """Create scan summary."""
        ...
        numerator = sum(severity_weights.get(sev, 0.25) * count for sev, count in severity_distribution.items())
